...
// Подключаемся к новой базе данных серверов
const db = new sqlite3.Database(path.join(__dirname, 'servers.db'), (err) => {
  ...
  // Создаем таблицы для новой базы серверов
  const createTablesSQL = `
    ...
    -- Таблица сообщений в чатах сервера (если нужно хранить локально на сервере)
  ...
  db.exec(createTablesSQL, (err) => {
    if (err) {
      console.error('Error creating tables in servers database:', err);
    } else {
      console.log('Servers database tables created successfully');
    }
    
    // Закрываем соединение
    db.close((err) => {
      if (err) {
        console.error('Error closing servers database:', err);
      } else {
        console.log('Servers database connection closed');
      }
    });
  });
});