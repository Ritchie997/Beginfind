...
- `GET /api/dashboard-summary` — сводка для страницы "Аналитика" (`src/services/dashboard-stats.js`):
  `users` (подтверждённые пользователи: всего и за 7 дней), `messages` (сообщения мессенджера **плюс**
  комментарии статей Ibripedia — всего и за 7 дней; комментарии удалённых статей не считаются) и
