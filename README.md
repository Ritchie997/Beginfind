...
- **Wiki-ссылки** `[[Название статьи]]` или `[[slug|текст]]`: подсвечиваются
  ...
  `|текст` сохраняется) переписывается во всех статьях, при удалении статьи
  ссылка на неё заменяется на «Удалено» (см. `refreshMentions`/`deleteArticle`
...
- **Цвет узлов по категории** статьи + легенда с расшифровкой (только на
  дашборде — `colorByCategory` в `renderGraph()`; локальная панель редактора
  фильтров и раскраски не получила, чтобы остаться компактной).
