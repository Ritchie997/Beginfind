 ...
 * исключены (в примерах кода `[текст]((как-в-css-переменной))` не должен считаться
 * ссылкой на статью).
 */
...
// Wiki-ссылка: [подпись]((цель)) или [подпись]((цель#якорь)) — см. blocks-renderer.js.
const WIKILINK_RE = /\[[^\]\n]*\]\(\(([^()#\n]+)(?:#[^()\n]*)?\)\)/g;
...
// Те же части, что и в WIKILINK_RE, но с подписью и якорем (вместе с "#") —
// чтобы заменить ссылку целиком, ничего не потеряв.
const WIKILINK_PARTS_RE = /\[([^\]\n]*)\]\(\(([^()#\n]+)(#[^()\n]*)?\)\)/g;

/**
 * Переписывает wiki-ссылки [подпись]((цель)) во всех markdown-строках
 * документа. Ссылка распознаётся так же, как при разборе/рендере — по
 * slugify(цель), а не по точному тексту, поэтому ((Дракон)), ((дракон)) и
 * ((drakon)) — одна и та же ссылка.
 *
 * rewrite({ slug, target, anchor, label }) получает slug цели, исходный
 * текст цели, якорь ("#раздел", с разделителем, или пустая строка) и подпись
 * (текст в квадратных скобках, может быть пустым) — и возвращает
 * строку-замену ссылки ЦЕЛИКОМ, включая подпись, либо null, если ссылку менять
 * не нужно (поэтому переименование статьи само дописывает подпись обратно, а
 * удаление — нет). Возвращает { doc, changed }.
 */
function rewriteWikiLinksInDocument(doc, slugify, rewrite) {
  ...
  const rewriteText = (text) => {
    if (!text) return text;
    return text.replace(WIKILINK_PARTS_RE, (full, label, target, anchor) => {
      const replacement = rewrite({ slug: slugify(target.trim()), target, anchor: anchor || '', label });
